...
# yeah no it's back to zero with centos
...
while ($errors == 1)
  ...
  cat /tmp/errors | grep 'shared lib'
  cat /tmp/errors | grep 'shared lib' \
      | sed 's/://g' \
  cat /tmp/errors | grep 'shared lib' \
      | sed 's/://g' \
...
# while loading shared libraries: libX11.so.6: cannot open shared object
# file: No such file or directory

# rpm -q --whatprovides libX11.so.6
# libX11-1.6.5-1.el7.i686
  ...
  if ( $errors == 1 ) then
    ...
    echo lib=$lib
    echo "Found missing package '$lib' maybe"
    if (! $?pkg) break
    ...
    set pkg=`rpm -q --whatprovides`
    echo "Found rpm package '$rpm' maybe"
