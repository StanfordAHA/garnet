...
class PassThroughCore(LakeCoreBase):
    ...
    def __init__(self,
        ...
        scan_name = "PassThrough"  # noqa "assigned but never used"
        ...
        conf_names.sort()
        with open("pass_through_cfg.txt", "w+") as cfg_dump:
            for idx, reg in enumerate(conf_names):
                write_line = f"(\"{reg}\", 0),  # {self.registers[reg].width}\n"
                cfg_dump.write(write_line)
        with open("pass_through_synth.txt", "w+") as cfg_dump:
            for idx, reg in enumerate(conf_names):
                write_line = f"{reg}\n"
                cfg_dump.write(write_line)
