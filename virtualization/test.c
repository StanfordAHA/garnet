...
int main() {
    int num_app = 1;
    struct KernelInfo *kernels[num_app];
    ...
    printf("%s\n", kernels[0]->placement_filename);
    printf("%d\n", kernels[0]->input_size[0]);
    printf("%d\n", kernels[0]->output_size[0]);

    if (glb_map(kernels, num_app) == 0) {
    ...
    struct PlaceInfo *p = kernels[0]->place_info;
    printf("num_groups: %d\n", p->num_groups);

    struct BitstreamInfo *b = get_bitstream_info(kernels[0]);
    printf("num_inputs: %d\n", get_num_inputs(p));
