...
def set_inputs(solver, input_symbols_coreir, input_symbols_pnr, bvsort16, id_to_name):
    ...
    input_pnr_symbols_to_names_reversed = {
        v: k for k, v in input_pnr_symbols_to_names.items()
    }
...
def create_property_term(
    ...
    # create an array for each 16 bit output
    for pair in output_pair_16_bit:
        for var in pair:
            var_name = str(var) + "_array"
            solver.create_fts_state_var(
                var_name,
    ...
    # create an array for each 1 bit output
    for pair in output_pair_1_bit:
        for var in pair:
            var_name = str(var) + "_array"
            solver.create_fts_state_var(
                var_name, solver.solver.make_sort(ss.sortkinds.ARRAY, bvsort16, bvsort1)
            )  # last 2 param = index_type, data_type, 1 bit data in this case

    # create counter for each output variable in CoreIR
    for coreir_output_var_name in coreir_output_datas:
        coreir_output_var = output_symbols_coreir[coreir_output_var_name]
        valid_name = f'{coreir_output_var_name.split("_write")[0]}_write_valid'
        assert valid_name in output_symbols_coreir
        coreir_valid = output_symbols_coreir[valid_name]
        ...
        )  # at the step where valid high and clk low
    ...
    for coreir_output_var_name in coreir_output_datas:
        ...
        out_pixel_count = solver.create_fts_state_var(
            f"output_count_{str(coreir_output_var_name)}", bvsort16
        )
        solver.fts.constrain_init(
            solver.create_term(
                solver.ops.Equal, out_pixel_count, solver.create_term(0, bvsort16)
    ...
    # create counter for each output variable in PnR
    for pnr_output_var_name in pnr_output_datas:
        pnr_output_var = output_symbols_pnr[pnr_output_var_name]
        data_name_lower = pnr_output_var_name.lower()
        valid_name = f'{data_name_lower.split("_17")[0]}_1'
        assert valid_name in output_symbols_pnr
        pnr_valid = output_symbols_pnr[valid_name]
        breakpoint()
        ...
        out_pixel_count = solver.create_fts_state_var(
            f"output_count_{str(pnr_output_var_name)}", bvsort16
        )
        solver.fts.constrain_init(
            solver.create_term(
                solver.ops.Equal, out_pixel_count, solver.create_term(0, bvsort16)
...
def verify_pnr(interconnect, coreir_file):
    ...
    nx_pnr = pnr_to_nx(routing_result_graph, cmod)
    ...
    set_inputs(solver, input_symbols_coreir, input_symbols_pnr, bvsort16, id_to_name)

    coreir_output_datas = []
    coreir_output_valids = []

    for output_var in output_symbols_coreir.keys():
        if "valid" == str(output_var)[-5:]:
            coreir_output_valids.append(output_var)
        else:
            coreir_output_datas.append(output_var)

    pnr_output_datas = []
    pnr_output_valids = []

    for output_var in output_symbols_pnr.keys():
        if "17" == str(output_var)[-2:]:
            pnr_output_datas.append(output_var)
        else:
            pnr_output_valids.append(output_var)

    create_property_term(
        solver,
        output_symbols_coreir,
        output_symbols_pnr,
        coreir_output_datas,
