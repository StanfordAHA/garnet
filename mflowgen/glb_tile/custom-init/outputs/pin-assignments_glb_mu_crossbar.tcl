...
# MU-GLB Crossbar pins
set mu_crossbar_addr_width 21
set mu_crossbar_tile_data_width 128

set mu_crossbar_pins []
lappend mu_crossbar_pins [get_object_name [get_ports "mu_crossbar_rd_en"]]
for {set i 0} {$i < $mu_crossbar_addr_width} {incr i} {
    lappend mu_crossbar_pins [get_object_name [get_ports "mu_crossbar_rd_addr_d[$i]"]]
}
lappend mu_crossbar_pins [get_object_name [get_ports "mu_crossbar_rd_data_valid_out"]]
for {set i 0} {$i < $mu_crossbar_tile_data_width} {incr i} {
    lappend mu_crossbar_pins [get_object_name [get_ports "mu_crossbar_rd_data_out[$i]"]]
}
...
# # Col 0: start at 5, end at width/4 - 5
# editPin -pin $bottom_col(0) -start [list [expr {($width/4)*0+5}] 0] -end [list [expr {($width/4)*1-5}] 0] -side BOTTOM -spreadType RANGE -spreadDirection counterclockwise -layer M3

# # Col1: start at 3width/4 + 5, end at width - 5
# editPin -pin $bottom_col(1) -start [list [expr {($width/4)*3+5}] 0] -end [list [expr {($width/4)*4-5}] 0] -side BOTTOM -spreadType RANGE -spreadDirection counterclockwise -layer M3


# All bottom pins
set bottom_pins []
lappend bottom_pins {*}$bottom_col(0)
lappend bottom_pins {*}$mu_crossbar_pins
lappend bottom_pins {*}$bottom_col(1)

editPin -pin $bottom_pins -start [list 5 0] -end [list [expr {$width - 5}] 0] -side BOTTOM -spreadType RANGE -spreadDirection counterclockwise -layer M3