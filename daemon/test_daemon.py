...
DAEMON_7x13 = DAEMON(7,13)
DAEMON_4x2  = DAEMON(4, 2)
...
#    test_double_launch() TODO
#    test_incompatible_daemon() TODO
#    TODO: more?

def kill_existing_daemon():
    print(f"\nKill existing daemon (p.stdout should say 'already dead')"); sys.stdout.flush()
...
def launch_daemon_and_verify(daemon=DAEMON):
    ...
    print(f'Launch a daemon, wait ten seconds'); sys.stdout.flush()
    p = subprocess.Popen(f'{DAEMON} launch'.split()); pid0 = p.pid
    time.sleep(10)
    pid1 = verify_daemon_running(); assert pid0 == pid1

    # bookmark
    # TODO look for e.g. 'Built 7x13'


    return pid0

def verify_no_daemon():
    print(f'\nCheck daemon status, should be "no daemon found"'); sys.stdout.flush()
...
# Problem: sigstop HANGS

# bookmark TODO clean up below, still need to do the verifies etc...

def test_daemon_use():
    from subprocess import Popen, PIPE
    tmpfile='/tmp/tmp'

    cmd=f'{DAEMON} force |& tee {tmpfile}'
...
def test_incompatible_daemon():
    return
    print('''
    try to use daemon but w/h do not match => error''')
    return False



# TODO use fluch=true or whatever instead of separate flush() call
...
# TODO
def test_initial_check(): return skip_test()
def test_loop(): return skip_test()
...
def test_kill(dbg=1):
    if announce(unit_test=True): return
...
# TODO
def test_status(): return skip_test()
def test_save_state0(): return skip_test()
...
# TODO
def test_daemon_wait(): return skip_test()
def test_die_if_daemon_exists(): return skip_test()
def test_check_for_orphans(): return skip_test()
def test_all_daemon_processes_except(): return skip_test()
def test_args_match_or_die(): return skip_test()
...
# TODO check all unit tests for announce(unit_test=True)
def test_sigstop():
    if announce(msg=": start writing dots to a file", unit_test=True): return
...
def test_grid_size(): return skip_test()
...
def announce(msg='', unit_test=False):
    import inspect
    curframe    = inspect.currentframe()
    callerframe = inspect.getouterframes(curframe, 2)
    caller = callerframe[1][3]
    # print('\n------------------------------------------------------------------------')
    print('\n========================================================================')
    print(f"--- TEST: {caller}(){msg}"); sys.stdout.flush()
    if unit_test and SKIP_UNIT_TESTS:
        print('- skipping unit tests'); return True
    else:
        return False
...
#     # this works i guess
#     # No it doesn't! Not if you pytest | tee e.g.
#     # alias pt='pytest --capture=no --noconftest --color=no daemon/test_daemon.py'
#     # pt -k test_daemon_use               => OKAY
#     # pt -k test_daemon_use | tee foo     => HANGS
...
# ========================================================================
# ========================================================================
# #     print(f'try again with the tmpfile "{tmpfile}"', flush=True)
# #     with open(tmpfile, 'r') as f: print(f.read())
# 
# #     # Try it with a tee
# #     # FAILS
# #     cmd=f'{DAEMON} force |& tee {tmpfile}'
# #     p1 = subprocess.Popen(['bash', '-c', cmd])
# #     time.sleep(1)
# #     print(f'reading output...?', flush=True)
# #     with open(tmpfile, 'r') as f: output_so_far = f.read()
# #     print(f'DAEMON OUTPUT SO FAR(1):\n{output_so_far}', flush=True)
# #     assert 'Built 7x13' in output_so_far
# #     return
# 
# #     # Unsolvable problem: readline HANGS
# #     proc = Popen(f'{DAEMON} force'.split(), stdout=PIPE, text=True)
# #     print('\nsleeping')
# #     time.sleep(5)
# #     print('done slept')
# #     for i in range(500):
...
# #             print(f'{i} {line.strip()}', flush=True)
# # 
# #     print("done counting")
# #     return
# 
...
# 
# 
#     proc = Popen('ping google.com'.split(), stdout=PIPE, text=True)
#     for i in range(5):
#         if proc.poll() is None:
#             line = proc.stdout.readline()
#             print(line.strip())
...
# 
# 
#     proc = subprocess.Popen(f'{DAEMON} force'.split(),
...
# 
# 
# #     with open("/tmp/foo", 'w') as tmpfile:
# #         while proc.poll() is None:
...
#     subprocess.run(f'{DAEMON} status'.split())
# 
# 
# 
#     print("USING NOW", flush=True)
#     # p2 = subprocess.run(f'{DAEMON} use --animal owl'.split(), text=True, capture_output=True)
#     p2 = subprocess.run(f'{DAEMON} use --animal owl'.split())
# 
#     print("STATUS", flush=True)
#     subprocess.run(f'{DAEMON} status'.split())
# 
#     # print(p2.stdout, flush=True); #assert('using animal: owl') in p2.stdout
# 
# 
#     with open(tmpfile, 'r') as f: output_so_far = f.read()
#     print(f'DAEMON OUTPUT SO FAR(2):\n{output_so_far}')
# 
# 
# 
...
# 
# 
# # assert 'Build 7x13' in '- hello here i am forcing a launch\n- found daemon pid 3237943, killing it now\n\nBuilt 7x13 garnet circuit number 3238650\n- using animal: mousie\n\n--- LAUNCHING 7x13 DAEMON 3238650\n\nDAEMON STOPS and waits...\n\n'
