...
def ungroup(top: Generator, *insts):
   ...
   # wires_to_remove = []
   # for wire in top.wires:
   #     # Find any wire that goes to the insts we want to ungroup
   #     for (ind, port) in enumerate(wire):
   #         if port.owner() in insts:
   #             # The top port must be the other port in the connection
   #             top_port = wire[ind - 1]
   #             wires_to_remove.append({'wire': wire, 'intermediate_port': port, 'top_port': top_port})
   #             # Now find the internal endpoints of this wire
   #             for internal_wire in port.owner().wires:
   #                 

   # # Now we have all of the wires entering instances we want to ungroup
   # for (wire, inst_port) in wires_to_remove:
   #     internal_wires_to_remove = []
   #     for internal_wire in port.owner().wires:
   #         for internal_port in internal_wire:
   #             if internal_port == inst_port:
   #                 internal_wires_to_remove.append(internal_wire)
   #                 break
   #             
    ...
    #for inst in insts:
    #    # Get all subinstances 
    #    # can do this by querying wires
    #    for wire in inst.wires:
    #        for port in wire:
    #            if port.owner() == inst:
    #                port_to_remove = port
    #                
    #    # Recreate all instances and connections in top
