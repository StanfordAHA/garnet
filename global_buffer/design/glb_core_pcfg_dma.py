...
class GlbCorePcfgDma(Generator):
    def __init__(self, params: GlobalBufferParams):
        ...
        self.num_cfg_cnt_r = self.var(
            "num_cfg_cnt_r", self.params.max_num_cfg_width)
        self.num_cfg_cnt_next = self.var(
            "num_cfg_cnt_next", self.params.max_num_cfg_width)
        self.addr_r = self.var("addr_r", self.params.glb_addr_width)
        self.addr_next = self.var("addr_next", self.params.glb_addr_width)
        ...
        self.rdrq_packet_rd_addr_r = self.var(
            "rdrq_packet_rd_addr_r", self.params.glb_addr_width)
        self.rdrq_packet_rd_addr_next = self.var(
            "rdrq_packet_rd_addr_next", self.params.glb_addr_width)
        self.rdrs_packet_rd_data_r = self.var(
            "rdrs_packet_rd_data_r", self.params.bank_data_width)
    ...
    def assign_cgra_cfg_output(self):
        ...
        self.wire(self.cgra_cfg_pcfg['addr'],
                  self.rdrs_packet_rd_data_r[self.params.cgra_cfg_data_width + self.params.cgra_cfg_addr_width - 1,
                                             self.params.cgra_cfg_data_width])
        self.wire(self.cgra_cfg_pcfg['data'],
                  self.rdrs_packet_rd_data_r[self.params.cgra_cfg_data_width - 1, 0])
