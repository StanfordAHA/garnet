...
class GlbCoreSramCfgCtrl(Generator):
    def __init__(self, params: GlobalBufferParams):
        ...
        self.bank2core_rd_data_valid_w = self.var(
            "bank2core_rd_data_valid_w", 1, size=self.params.banks_per_tile)
        self.bank2core_rd_data_w = self.var(
            "bank2core_rd_data_w", self.params.axi_data_width, size=self.params.banks_per_tile)
        self.rd_data_valid_w = self.var("rd_data_valid_w", 1)
        self.rd_data_w = self.var("rd_data_w", self.params.axi_data_width)
    ...
    def tile_id_match_logic(self):
        self.tile_id_match = self.if_sram_cfg_wst_s.wr_addr[self.params.bank_addr_width + self.params.bank_sel_addr_width +
                                                            self.params.tile_sel_addr_width - 1, self.params.bank_addr_width + self.params.bank_sel_addr_width] == self.glb_tile_id
