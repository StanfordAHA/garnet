...
class GlbCoreStrmMux(Generator):
    def __init__(self, params: GlobalBufferParams):
        ...
        self.data_g2f_dma = self.input(
            "data_g2f_dma", width=self.params.cgra_data_width)
        self.data_valid_g2f_dma = self.input(
            "data_valid_g2f_dma", width=1)
        self.data_g2f = self.output(
            "data_g2f", width=self.params.cgra_data_width, size=self.params.cgra_per_glb)
        self.data_valid_g2f = self.output(
            "data_valid_g2f", width=1, size=self.params.cgra_per_glb)

        self.data_f2g_dma = self.output(
            "data_f2g_dma", width=self.params.cgra_data_width)
        self.data_valid_f2g_dma = self.output(
            "data_valid_f2g_dma", width=1)
        self.data_f2g = self.input(
            "data_f2g", width=self.params.cgra_data_width, size=self.params.cgra_per_glb)
        self.data_valid_f2g = self.input(
            "data_valid_f2g", width=1, size=self.params.cgra_per_glb)

        self.cfg_data_network_g2f_mux = self.input(
            "cfg_data_network_g2f_mux", self.params.cgra_per_glb)
        self.cfg_data_network_f2g_mux = self.input(
            "cfg_data_network_f2g_mux", self.params.cgra_per_glb)

        # local variables
        self.data_g2f_int = self.var(
            "data_g2f_int", width=self.params.cgra_data_width, size=self.params.cgra_per_glb)
        self.data_valid_g2f_int = self.var(
            "data_valid_g2f_int", width=1, size=self.params.cgra_per_glb)
    ...
    def pipeline(self):
        if self.reset:
            for i in range(self.params.cgra_per_glb):
                self.data_g2f[i] = 0
