...
class GlbTileSramCfgCtrl(Generator):
    def __init__(self, _params: GlobalBufferParams):
        ...
        self.bank2ctrl_rd_data_valid_w = self.var("bank2ctrl_rd_data_valid_w", 1, size=self._params.banks_per_tile)
        self.bank2ctrl_rd_data_w = self.var(
            "bank2ctrl_rd_data_w", self._params.axi_data_width, size=self._params.banks_per_tile)
        ...
        for i in range(self._params.banks_per_tile):
            self.add_always(self.if_sram_cfg_ctrl2bank_logic, i=i)
    ...
    @always_comb
    def if_sram_cfg_ctrl2bank_logic(self, i):
        if self.tile_id_match:
            self.if_sram_cfg_ctrl2bank_m[i].wr_en = ((self.if_sram_cfg_wst_s.wr_addr[self._params.bank_addr_width
                                                                                     + self._params.bank_sel_addr_width
                                                                                     - 1, self._params.bank_addr_width]
                                                      == i) & self.if_sram_cfg_wst_s.wr_en)
            self.if_sram_cfg_ctrl2bank_m[i].wr_addr = self.if_sram_cfg_wst_s.wr_addr[self._params.bank_addr_width - 1,
                                                                                     0]
            self.if_sram_cfg_ctrl2bank_m[i].wr_data = self.if_sram_cfg_wst_s.wr_data
            self.if_sram_cfg_ctrl2bank_m[i].rd_en = ((self.if_sram_cfg_wst_s.rd_addr[self._params.bank_addr_width
                                                                                     + self._params.bank_sel_addr_width
                                                                                     - 1,
                                                                                     self._params.bank_addr_width] == i)
                                                     & self.if_sram_cfg_wst_s.rd_en)
            self.if_sram_cfg_ctrl2bank_m[i].rd_addr = self.if_sram_cfg_wst_s.rd_addr[self._params.bank_addr_width - 1,
                                                                                     0]
        else:
