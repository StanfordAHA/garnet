...
class GlbRingSwitchMultiBank(Generator):
    def __init__(self, _params: GlobalBufferParams, wr_channel=True, rd_channel=True):
        ...
        if self.wr_channel:
            self.wr_packet_w2e_wsti = self.input("wr_packet_w2e_wsti", self.header.wr_packet_t, size=self._params.cgra_per_glb)
            self.wr_packet_e2w_wsto = self.output("wr_packet_e2w_wsto", self.header.wr_packet_t, size=self._params.cgra_per_glb)
            self.wr_packet_e2w_esti = self.input("wr_packet_e2w_esti", self.header.wr_packet_t, size=self._params.cgra_per_glb)
            self.wr_packet_w2e_esto = self.output("wr_packet_w2e_esto", self.header.wr_packet_t, size=self._params.cgra_per_glb)
            self.wr_packet_dma2ring = self.input("wr_packet_dma2ring", self.header.wr_packet_t, size=self._params.cgra_per_glb)
            self.wr_packet_ring2bank = self.output("wr_packet_ring2bank", self.header.wr_packet_t, size=self._params.cgra_per_glb)
        ...
        if self.rd_channel:
            ...
            self.rdrs_packet_w2e_wsti = self.input("rdrs_packet_w2e_wsti", self.header.rdrs_packet_t, size=self._params.cgra_per_glb)
            self.rdrs_packet_e2w_wsto = self.output("rdrs_packet_e2w_wsto", self.header.rdrs_packet_t, size=self._params.cgra_per_glb)
            self.rdrs_packet_e2w_esti = self.input("rdrs_packet_e2w_esti", self.header.rdrs_packet_t, size=self._params.cgra_per_glb)
            self.rdrs_packet_w2e_esto = self.output("rdrs_packet_w2e_esto", self.header.rdrs_packet_t, size=self._params.cgra_per_glb)
            ...
            self.rdrs_packet_bank2ring = self.input("rdrs_packet_bank2ring", self.header.rdrs_packet_t, size=self._params.cgra_per_glb)
            self.rdrs_packet_ring2dma = self.output("rdrs_packet_ring2dma", self.header.rdrs_packet_t, size=self._params.cgra_per_glb)
        ...
        # local variables
        if self.wr_channel:
            self.wr_packet_w2e_wsti_muxed = self.var("wr_packet_w2e_wsti_muxed", self.header.wr_packet_t, size=self._params.cgra_per_glb)
            self.wr_packet_w2e_esto_w = self.var("wr_packet_w2e_esto_w", self.header.wr_packet_t, size=self._params.cgra_per_glb)
            self.wr_packet_e2w_esti_muxed = self.var("wr_packet_e2w_esti_muxed", self.header.wr_packet_t, size=self._params.cgra_per_glb)
            self.wr_packet_e2w_wsto_w = self.var("wr_packet_e2w_wsto_w", self.header.wr_packet_t, size=self._params.cgra_per_glb)
            self.wr_packet_ring2bank_w = self.var("wr_packet_ring2bank_w", self.header.wr_packet_t, size=self._params.cgra_per_glb)
        ...
        if self.rd_channel:
            ...
            self.rdrs_packet_w2e_wsti_muxed = self.var("rdrs_packet_w2e_wsti_muxed", self.header.rdrs_packet_t, size=self._params.cgra_per_glb)
            self.rdrs_packet_w2e_esto_w = self.var("rdrs_packet_w2e_esto_w", self.header.rdrs_packet_t, size=self._params.cgra_per_glb)
            self.rdrs_packet_e2w_esti_muxed = self.var("rdrs_packet_e2w_esti_muxed", self.header.rdrs_packet_t, size=self._params.cgra_per_glb)
            self.rdrs_packet_e2w_wsto_w = self.var("rdrs_packet_e2w_wsto_w", self.header.rdrs_packet_t, size=self._params.cgra_per_glb)
            self.rdrs_packet_ring2dma_w = self.var("rdrs_packet_ring2dma_w", self.header.rdrs_packet_t, size=self._params.cgra_per_glb)
        ...
        if self.wr_channel:
            for i in range(self._params.cgra_per_glb):
                self.add_always(self.wr_packet_switch_logic, i=i)

        if self.rd_channel:
            self.add_always(self.rdrq_packet_switch_logic)
            for i in range(self._params.cgra_per_glb):
                self.add_always(self.rdrs_packet_switch_logic, i=i)
    ...
    @always_comb
    def wr_packet_switch_logic(self, i):
        if (self.wr_packet_dma2ring[i]['wr_en'] == 1):
            if (self.wr_packet_dma2ring[i]['wr_addr'][self.tile_sel_msb, self.tile_sel_lsb] == self.glb_tile_id):
                self.wr_packet_ring2bank_w[i] = self.wr_packet_dma2ring[i]
                self.wr_packet_w2e_esto_w[i] = 0
            else:
                self.wr_packet_ring2bank_w[i] = 0
                self.wr_packet_w2e_esto_w[i] = self.wr_packet_dma2ring[i]
        elif (self.wr_packet_w2e_wsti_muxed[i]['wr_en'] == 1):
            if (self.wr_packet_w2e_wsti_muxed[i]['wr_addr'][self.tile_sel_msb, self.tile_sel_lsb] == self.glb_tile_id):
                self.wr_packet_ring2bank_w[i] = self.wr_packet_w2e_wsti_muxed[i]
                self.wr_packet_w2e_esto_w[i] = 0
            else:
                self.wr_packet_ring2bank_w[i] = 0
                self.wr_packet_w2e_esto_w[i] = self.wr_packet_w2e_wsti_muxed[i]
        else:
            self.wr_packet_ring2bank_w[i] = 0
            self.wr_packet_w2e_esto_w[i] = 0
        self.wr_packet_e2w_wsto_w[i] = self.wr_packet_e2w_esti_muxed[i]
    ...
    @always_comb
    def rdrs_packet_switch_logic(self, i):
        if (self.rdrs_packet_bank2ring[i]['rd_data_valid'] == 1):
            self.rdrs_packet_w2e_esto_w[i] = self.rdrs_packet_bank2ring[i]
        else:
            if (self.rdrs_packet_w2e_wsti_muxed[i]['rd_data_valid'] & self.cfg_ld_dma_on):
                self.rdrs_packet_w2e_esto_w[i] = 0
            else:
                self.rdrs_packet_w2e_esto_w[i] = self.rdrs_packet_w2e_wsti_muxed[i]

        if (self.rdrs_packet_w2e_wsti_muxed[i]['rd_data_valid'] & self.cfg_ld_dma_on):
            self.rdrs_packet_ring2dma_w[i] = self.rdrs_packet_w2e_wsti_muxed[i]
        else:
            self.rdrs_packet_ring2dma_w[i] = 0
        self.rdrs_packet_e2w_wsto_w[i] = self.rdrs_packet_e2w_esti_muxed[i]
    ...
    def add_ring2bank_clk_en(self):
        if self.wr_channel:
            ...
            self.add_child("ring2bank_wr_clk_en_gen",
                           ...
                           clk=self.clk,
                           reset=self.reset,
                           enable=self.wr_packet_ring2bank_w[0]['wr_en'],
