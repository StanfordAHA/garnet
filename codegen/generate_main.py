...
{{
  ...
  uint32_t mask = (1 << AHASOC_PCTRL_UART0_Pos) | (1 << AHASOC_PCTRL_DMA0_Pos) | (1 << AHASOC_PCTRL_CGRA_Po
...
'''
  ...
  for(int run=0; run < runs; run++){{
    ...
    while(glc_read_reg(GLC_STRM_F2G_ISR_R) != stream_pulse_f2g){{
        //cnt++;
    }}
...
'''
    ...
    for i, output in enumerate(output_list):
        f.write(f'    glb_reg_write(0x100 * {i} + GLB_ST_DMA_HEADER_0_START_ADDR_R, 0x20000 + 0x40000*{i} + {output}_idx*2);\n')
